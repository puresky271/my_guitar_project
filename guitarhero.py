...
st.markdown("""
    ...
    /* 简约的信息卡片容器 */
    .metric-container {
        background-color: rgba(255, 255, 255, 0.05); /* 深色半透明背景 */
...
def render_sync_player(audio_bytes):
    """
    渲染一个标准的 HTML5 Audio 播放器，并让下方的 WaveSurfer 自动同步
    """
...
with st.sidebar:
    ...
    pluck_position = st.slider(
        "拨弦位置（靠琴桥 ⇄ 靠指板）",
        0.08, 0.35, step=0.01, key="pluck_position",
        help="决定音色是清脆还是温暖。数值越小越清脆（靠琴桥），数值越大越圆润（靠指板）。"
    )

    body_mix = st.slider(
        "琴箱共鸣强度",
        0.0, 0.6, step=0.02, key="body_mix",
        help="越大越有木头味（Boxy），但过大会导致声音变闷。"
    )

    reflection = st.slider(
        "空间反射感",
        0.0, 0.3, step=0.01, key="reflection",
        help="模拟琴体内部的回响，增加空气感。"
    )

    brightness = st.slider(
        "弦的亮度",
        0.2, 0.8, step=0.02, key="brightness",
        help="控制弦振动的高频保留时间，值越大声音越明亮。"
    )

    coupling = st.slider(
        "弦间共振（串扰）",
        0.0, 0.01, step=0.0005, key="coupling",
    ...
    if uploaded_file:
        with st.container():
            ...
            <div class="metric-container">
                <div><strong>📄 文件:</strong> <span style="font-family: monospace;">{uploaded_file.name}</span></div>
            ...
            if st.button("🎧 GuitarHero，启动！", type="primary", use_container_width=True):
                with st.status("正在进行流式物理计算...", expanded=True) as status:
                    ...
                    audio_bytes, _ = midi_to_audio(
                        uploaded_file,
                        brightness, pluck_position, body_mix, reflection, coupling
                    )
